...
def pad_to_same_size(img1, img2):
    ...
    img1_padded = F.pad(img1, (0, max_w - w1, 0, max_h - h1))
    img2_padded = F.pad(img2, (0, max_w - w2, 0, max_h - h2))
...
def cal_laplacian_loss(
    foreground_img, foreground_mask, blended_img, background_mask, dx, dy
):
    """
    ...
    # Ensure the foreground and blended images have the same size
    foreground_img, blended_img = pad_to_same_size(foreground_img, blended_img)
    foreground_mask, background_mask = pad_to_same_size(
        foreground_mask, background_mask
    )

    # Shift the foreground image and mask by dx and dy
    foreground_img_shifted = torch.roll(foreground_img, shifts=(dy, dx), dims=(2, 3))
    foreground_mask_shifted = torch.roll(foreground_mask, shifts=(dy, dx), dims=(2, 3))

    # Initialize loss
    loss = torch.tensor(0.0, device=foreground_img.device)

    # Compute Laplacian loss for each channel
    for c in range(3):
        foreground_laplacian = torch.nn.functional.conv2d(
            foreground_img_shifted[:, c : c + 1, :, :], laplacian_kernel, padding=1
        )
        blended_laplacian = torch.nn.functional.conv2d(
            blended_img[:, c : c + 1, :, :], laplacian_kernel, padding=1
        )
        # Only compute the loss within the selected mask region
        loss += torch.sum(
            (
                (foreground_laplacian - blended_laplacian) ** 2
                * foreground_mask_shifted
                * background_mask
            )
        )
    loss *= 10000
...
def blending(
    ...
    for step in range(iter_num):
        ...
        loss = cal_laplacian_loss(
            fg_img_tensor, fg_mask_tensor, blended_img_for_loss, bg_mask_tensor, dx, dy
        )
...
""",
    ...
    foreground_image_original.change(
        fn=lambda img: (img, initialize_polygon()),
