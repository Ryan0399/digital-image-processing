...
class GaussianTrainer:
    def __init__(
        ...
        optable_params = [
            {"params": [self.model.positions], "lr": 0.0016, "name": "xyz"},
    ...
    def visualize_rendering(self, dataset, save_vid_path: str, num_frames: int = 300):
        ...
        for data_item in tqdm(dataset, desc="Rendering frames"):
            ...
            vis = cv2.cvtColor(
                np.concatenate((ori_img, frame), axis=1), cv2.COLOR_RGB2BGR
            )
            # Write frame
            out.write(vis)
    ...
    def train(self, train_loader: DataLoader):
        ...
        for epoch in range(self.config.num_epochs):
            ...
            if epoch % self.config.debug_every == 0:
                ...
                debug_batch = {
                    k: (
                        torch.stack([b[k] for b in debug_batches], 0)
...
def parse_args():
    ...
    # Debug parameters
    parser.add_argument(
        "--debug_every", type=int, default=1, help="Save debug images every N epochs"
    )
    parser.add_argument(
        "--debug_samples",
        type=int,
...
def main():
    ...
    model = GaussianModel(
        points3D_xyz=dataset.points3D_xyz, points3D_rgb=dataset.points3D_rgb
    )
    ...
    trainer.visualize_rendering(
        dataset, os.path.join(args.checkpoint_dir, "debug_rendering.mp4")
    )
